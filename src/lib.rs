...
fn interval_seconds(sample: &Sample, fallback_interval_minutes: u32) -> i64 {
    i64::from(
        sample
            .interval_minutes
            .unwrap_or(fallback_interval_minutes)
            .max(1),
    ) * 60
}
...
pub fn group_samples(
    ...
    for index in 1..samples.len() {
        ...
        let prev_interval = interval_seconds(prev, fallback_interval_minutes);
    ...
    runs.iter()
        ...
        .map(|(run_index, &(start_index, end_index))| {
            ...
            let interval_end = last.timestamp
                + chrono::Duration::seconds(interval_seconds(last, fallback_interval_minutes));
            let next_start = runs
                .get(run_index + 1)
