...
Both views share the compact sidebar, status, button styles, colors, type, and content width. On small screens, navigation becomes horizontal and the activity table becomes labelled rows. All controls have accessible names. All observations are samples, never presented as exact working time.

## Interaction

- Dashboard: date navigation, search and app/project filters, record classification, editable memo export.
