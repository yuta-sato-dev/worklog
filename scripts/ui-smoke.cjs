...
(async()=>{
 ...
 const layouts=[];
 for(const width of [720,768,1000,1120]){
 ...
 assert.deepEqual(await page.locator('#title-segments button').evaluateAll(buttons=>buttons.map(b=>b.textContent)),['Worklog','プロジェクトの作業','Worklog — プロジェクトの作業','候補にない文字列を入力']);
 await page.getByRole('button',{name:'Worklog',exact:true}).click();
 assert.equal(await page.locator('[name=contains]').inputValue(),'Worklog');
 assert.equal(await page.getByRole('button',{name:'Worklog',exact:true}).getAttribute('aria-pressed'),'true');
 assert.match(await page.locator('#classify-match').textContent(),/1件/);
 await page.getByRole('button',{name:'候補にない文字列を入力'}).click();
 assert.equal(await page.locator('#custom-contains-label').evaluate(e=>e.hidden),false);
 assert.equal(await page.getByRole('button',{name:'候補にない文字列を入力'}).getAttribute('aria-expanded'),'true');
 await page.locator('#custom-contains').fill('候補にない条件');
 assert.match(await page.locator('#classify-warning').textContent(),/この記録のタイトルには含まれていないため/);
 await page.locator('[name=project]').fill('External project');await page.locator('#rule-form button[type=submit]').click();await page.waitForFunction(()=>!document.querySelector('#classify').open);assert.match(await page.locator('#rules').textContent(),/External project/);
 await page.locator('.activity-row button').first().click();assert.equal(await page.locator('#classify').evaluate(e=>e.open),true);
 await page.getByRole('button',{name:'候補にない文字列を入力'}).click();
 await page.locator('#custom-contains').fill('Worklog');
 assert.equal(await page.locator('#classify-warning').textContent(),'');
 ...
 await page.getByRole('button',{name:'プロジェクトの作業',exact:true}).click();
 assert.match(await page.locator('#classify-warning').textContent(),/既存のルール「Worklog」/);
 await page.locator('#cancel-rule').click();await page.waitForFunction(()=>!document.querySelector('#classify').open);
 await page.evaluate(()=>window.__TAURI__.core.invoke('add_rule',{app:'Firefox',contains:'ChatGPT',project:'Chat project'}));
 await page.locator('#refresh').click();await page.waitForFunction(()=>[...document.querySelectorAll('.activity-row')].filter(row=>row.querySelectorAll('button').length===2).length>=2);
 for(const width of [720,768,1000,1120]){
  await page.setViewportSize({width,height:900});
  const overflow=await page.evaluate(()=>[...document.querySelectorAll('body *')].filter(e=>e.getClientRects().length && !e.closest('dialog') && !e.classList.contains('sr-only')).map(e=>({tag:e.tagName,id:e.id,cls:e.className,x:e.getBoundingClientRect().x,right:e.getBoundingClientRect().right,scroll:e.scrollWidth,client:e.clientWidth})).filter(e=>e.x<-.5||e.right>innerWidth+.5));
  assert.deepEqual(overflow,[],`classified overflow at ${width}: ${JSON.stringify(overflow)}`);
  const columns=await page.evaluate(()=>{
   const rows=[...document.querySelectorAll('.activity-row')];
   return {
    projectXs:rows.map(row=>row.querySelector('.project-cell').getBoundingClientRect().x),
    titleWidths:rows.map(row=>row.children[1].getBoundingClientRect().width),
    headerProjectX:document.querySelector('.activity-columns span:nth-child(3)').getBoundingClientRect().x
   };
  });
  const near=(a,b)=>Math.abs(a-b)<=1;
  assert.ok(columns.projectXs.every(x=>near(x,columns.projectXs[0])),`project x mismatch at ${width}: ${JSON.stringify(columns.projectXs)}`);
  assert.ok(columns.titleWidths.every(w=>near(w,columns.titleWidths[0])),`title width mismatch at ${width}: ${JSON.stringify(columns.titleWidths)}`);
  assert.ok(near(columns.projectXs[0],columns.headerProjectX),`project header mismatch at ${width}: ${JSON.stringify(columns)}`);
 }
 await page.getByRole('button',{name:/分類解除/}).first().click();await page.waitForFunction(()=>document.querySelector('#unclassify').open);
 assert.match(await page.locator('#unclassify').textContent(),/Worklog/);assert.match(await page.locator('#unclassify').textContent(),/1件/);
 assert.equal(await page.locator('#unclassify-form button[type=submit]').isDisabled(),false);
 await page.locator('#unclassify-form button[type=submit]').click();await page.waitForFunction(()=>!document.querySelector('#unclassify').open);
 await page.getByRole('button',{name:/分類解除/}).first().click();await page.waitForFunction(()=>document.querySelector('#unclassify').open);
 assert.equal(await page.locator('#unclassify-form button[type=submit]').isDisabled(),false);
 await page.locator('#unclassify-form button[type=submit]').click();await page.waitForFunction(()=>!document.querySelector('#unclassify').open);
 assert.equal(await page.evaluate(()=>window.calls.some(c=>c.cmd==='delete_rule')),true);
