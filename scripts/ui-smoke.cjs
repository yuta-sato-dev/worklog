...
(async()=>{
 ...
 await page.locator('.activity-row button').first().click();assert.equal(await page.locator('#classify').evaluate(e=>e.open),true);
 assert.equal(await page.locator('[name=contains]').getAttribute('readonly'),'');
 assert.equal(await page.locator('[name=contains]').inputValue(),'プロジェクトの作業');
 assert.deepEqual(await page.locator('#title-segments button').evaluateAll(buttons=>buttons.map(b=>b.textContent)),['Worklog','プロジェクトの作業','Worklog — プロジェクトの作業']);
 ...
 assert.deepEqual(errors,[]);console.log(JSON.stringify({passed:true,layouts,checks:['unsafe title rendered as text','newest timeline first','duration and range','title unavailable cannot classify','report page','search','classification candidates include full title','classification readonly initial segment','classification save','duplicate classification warning','overlap classification warning','pause/resume','accessibility permission','settings save','autostart','export oldest first','no runtime errors'],screenshots:path.join(screenshotDir, 'worklog-{width}.png')},null,2));
