...
(async()=>{
 ...
 assert.deepEqual(errors,[]);console.log(JSON.stringify({passed:true,layouts,checks:['unsafe title rendered as text','search','classification save','pause/resume','settings save','autostart','export','no runtime errors'],screenshots:path.join(screenshotDir, 'worklog-{width}.png')},null,2));
